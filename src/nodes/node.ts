...
export function waitForElement(conf : SeleniumNodeDef, msg : SeleniumMsg) : Observable<string | WebElement>{
    return new Observable<string | WebElement> ((subscriber) => {
        ...
        const timeout : number = parseInt(conf.timeout? conf.timeout : msg.timeout , 10);
        const target : string = conf.target ? conf.target : msg.target;
        const selector : string = conf.selector ? conf.selector : msg.selector;
